...
static ncclResult_t nccl_ucx_get_pci_path(char* devName, char** path, int* realPort) {
...
ncclResult_t nccl_ucx_init(ncclDebugLogger_t logFunction) {
  ...
  if (ncclNIbDevs == -1) {
    ...
    if (ncclNIbDevs == -1) {
	  ...
	  NCCLCHECK(nccl_ucx_get_pci_path(ncclIbDevs[ncclNIbDevs].devName, &ncclIbDevs[ncclNIbDevs].pciPath, &ncclIbDevs[ncclNIbDevs].realPort));
    ...
    if (ncclNIbDevs == 0) {
      ...
      line[1023] = '\0';
      char addrline[1024];
      INFO(NCCL_INIT|NCCL_NET, "NET/IB : Using%s ; OOB %s:%s", line, if_name, socketToString(&nccl_ucx_if_addr.sa, addrline));
    }
    pthread_mutex_unlock(&nccl_ucx_lock);
  }
  return ncclSuccess;
